...
The mean is `r round(steps_mean)` steps.  
The median is `r steps_median` steps.
...
The interval with the most steps is `r max_interval$interval`.  
It has `r round(max_interval$steps,1)` steps.
...
There are `r na_count` intervals with missing step data.
